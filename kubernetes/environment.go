...
func (instance *Environment) RegisterFlags(fg kingpin.FlagGroup) {
	fg.Flag("kubeconfig", "Defines the location of the kubeconfig."+
	...
	fg.Flag("context", "Defines context which you try to access of the kubeconfig."+
		" This value is ignored if kbueconfig 'incluster' is used.").
		PlaceHolder("<context>").
		Short('c').
		Envar("CONTEXT").
		StringVar(&instance.Context)
	fg.Flag("namespace", "Defines namespace where it service is bound to/running in."+
		" This value is ignored if kbueconfig 'incluster' is used.").
...
func (instance *Environment) init() {
	instance.initOnce.Do(func() {
		if instance.tokenFile == "" {
			instance.tokenFile = ServiceTokenFile
		}
		if instance.rootCAFile == "" {
			instance.rootCAFile = ServiceRootCAFile
		}
		if instance.namespaceFile == "" {
			instance.namespaceFile = ServiceNamespace
		}
	})
}
